...
from .serializers import ModelVersionSerializer, PredictionSerializer, PipelineRunSerializer
...
def trigger_retrain(request):
    try:
        response = requests.post(f'{settings.AIRFLOW_API_URL}/dags/churn_prediction_pipeline/dagRuns', json={'conf': {}}, auth=(settings.AIRFLOW_ADMIN_USER, settings.AIRFLOW_ADMIN_PASSWORD), timeout=10)
        if response.status_code in [200, 201]:
            return Response({'message': 'DAG triggered successfully'}, status=status.HTTP_200_OK)
        return Response({'error': 'Failed to trigger DAG'}, status=status.HTTP_502_BAD_GATEWAY)
